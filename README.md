...
Sessions allow the user to retain authorization access to restricted paths without having to authenticate them for every single request made via the browser.

2. What does bcrypt do to help us store passwords in a secure manner.

Bcrypt helps hash the password, which is similar to encryption but more secure.

3. What does bcrypt do to slow down attackers?

Bcrypt includes "salts" (superfluous data essentially) and purposely hashes passwords slowly. The developer using bcrypt can add hashing rounds to increase this effect.

4. What are the three parts of the JSON Web Token?

the header: algorithm and token type
the payload: claims and other data we want passed (e.g. user id)
the signature: the header converted to a string and signed with a secret
